...
void main() {
  ...
  group('ORDER-01 Dine-In Table Order E2E API Flow', () {
    ...
    test('Step 1 & 2: Fetch tables and select an Available table', () async {
      ...
      final tables = response.data['data'] as List<dynamic>;
      expect(tables, isNotEmpty);
      
      // Select the first table that is not the counter table
      final table = tables.firstWhere((t) => t['id'] != '00000000-0000-0000-0000-000000000001') as Map<String, dynamic>;
      tableId = table['id'] as String;
      print('Selected Table: ${table['table_number']} (ID: $tableId)');
    });

    test('Step 3: Resolve QR Session token for the selected table (Seat Guests)', () async {
    ...
    test('Step 4 & 5: Load Menu, create cart and add 3 items to cart', () async {
      // Fetch menu items
    ...
    test('Step 7 & 8: Update cart item (Add kitchen note)', () async {
      ...
      final cartDetail = cartResponse.data['data'] as Map<String, dynamic>;
      final cartItemsList = cartDetail['items'] as List<dynamic>;
      expect(cartItemsList, isNotEmpty);
      final cartItem = cartItemsList.first as Map<String, dynamic>;
      final cartItemId = cartItem['id'] as String;
      final cartItemVersion = cartItem['version_num'] as int;

      // Update item notes (e.g. "No onion")
      final mutationId = uuid.v4();
      final updateResponse = await dio.patch(
        ...
        data: {
          ...
          'payload': {
            'quantity': 2,
            'item_notes': 'No onion',
            'version_num': cartItemVersion,
          },
        },
      );

      expect(updateResponse.statusCode, anyOf(200, 201));
      final updatedCartDetail = updateResponse.data['data'] as Map<String, dynamic>;
      final updatedCart = updatedCartDetail['cart'] as Map<String, dynamic>;
      currentRevision = updatedCart['version_num'] as int;
      expect(updatedCartDetail['items'].first['item_notes'], 'No onion');
      print('Updated Item Note to "No onion". New Cart Revision: $currentRevision');
    });

    test('Step 9: Checkout Order (Send to Kitchen)', () async {
      final mutationId = uuid.v4();
      final checkoutResponse = await dio.post(
        ...
        data: {
          ...
          'payload': {
            'cartId': cartId,
            'tableId': tableId,
            'orderNotes': 'Kitchen note: Urgent',
          },
        },
      );
      ...
      print('🎉 Order checked out successfully! Order ID: ${order['id']}');
    });
  });
}
